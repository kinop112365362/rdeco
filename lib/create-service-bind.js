    ...
    serviceKeys.forEach(serviceKey => {
      ...
      if (serviceIsArray) {
        serviceBind[serviceKey] = (...args) => {
          rcsHooks.service.beforeRunWithEnhancer(serviceKey, service[serviceKey], callEffectContext.name, ...args);
          const res = service[serviceKey][1].call({
            ...callEffectContext, service: serviceBind
          }, ...args);
          rcsHooks.service.afterRunWithEnhancer(res, serviceKey, service[serviceKey]);
          return res;
        };
      } else {
        serviceBind[serviceKey] = (...args) => {
          rcsHooks.service.beforeRun(serviceKey, callEffectContext.name, ...args);
