...
const getReducerType = stateKey => `set${stateKey.charAt(0).toUpperCase()}${stateKey.slice(1)}`;
...
export const createReducer = stateKeys => state => {
  const reducer = {
...
export function createRC(stateKeys, dispatch, state) {
  const rc = {
    setState(nextState) {
      ...
      if (ghostKeys.length) {
        throw new Error(`不存在的 state => [${ghostKeys.toString()}], 请确保setState中更新的state在initState中已经声明`);
      } else {
