...
export function createStoreTakenMembraneHook(storeConfig, membraneStoreConfig) {
  ...
  return function useMembraneStore(props) {
    ...
    }, initArgs => membraneStoreStateConfig.init(storeStateConfig.init(initArgs)));
    ...
    const sharedContext = createSharedContext(mergedStateKeys, dispatch, state, mergedRef);
    const superBindContext = createStoreBindContext(context, storeConfig, sharedContext);
    const finlayStore = createStoreBindContext(context,
      membraneStoreConfig,
      sharedContext,
      superBindContext, props);
    return Object.freeze({
      state: pick(finlayStore.state, storeStateConfig.stateKeys),
      refs: pick(finlayStore.refs, storeStateConfig.refKeys),
      controller: pick(finlayStore.controller, Object.keys(storeConfig.controller)),
