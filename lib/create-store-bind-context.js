...
export function createStoreBindContext(context, {
  name,
  service,
  controller,
  view }, { rc, refs, state }, superBindContext, props) {
  ...
  if (superBindContext) {
    ...
    const serviceBind = createServiceBind(service, Object.freeze(callServiceContext));
    callServiceContext.super.controller = superBindContext.controller;
    const controllerBind = createControllerBind(controller, Object.freeze({
      ...callServiceContext,
      service: serviceBind,
      rc,
    }));
  ...
  const viewBind = createViewBind(view, Object.freeze(getViewContext(controllerBind)));
  return Object.freeze({
    rc,
    service: serviceBind,
    state,
